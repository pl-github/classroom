...
use Code\AnalyzerBundle\Writer\WriterResolver;

class WriterResolverTest extends \PHPUnit_Framework_TestCase
{
    public function testResolveReturnsSupportingWriter()
    {
        $writer1 = $this->getMockBuilder('Code\AnalyzerBundle\Writer\WriterInterface')
            ->getMock();

        $writer2 = $this->getMockBuilder('Code\AnalyzerBundle\Writer\WriterInterface')
            ->getMock();
